...
$('input[type="checkbox"][name="rent"]').change( function (){
    if($(this).get(0).checked){
        $('select[name="due_date"]').attr('disabled', false);
        $('select[name="deadline"]').attr('disabled', false);
    }else{
        $('select[name="due_date"]').attr('disabled', true);
        $('select[name="deadline"]').attr('disabled', true);
    }
})

// ENABLE INPUT TO PRICE SALE
